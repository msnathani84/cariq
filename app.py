...
# ── OPTIONAL: OpenCV (gracefully disabled if not installed) ───────────────────
try:
    import cv2
    ...
    print("[CarIQ]  → Run: pip install opencv-python-headless pillow")
    print("[CarIQ]  → Module 4 (condition) and Module 5 (fraud) will be skipped until installed.")
...
try:
    ...
    print("[CarIQ]  → Falling back to rule-based price estimation for Module 1")
    print("[CarIQ]  → Module 3 depreciation will return None until models/ folder is present")

# ── MARKET ADJUSTMENT CONFIG — city tier + body type multipliers ──────────────
# FIX 1: Load market_adjustment_config.pkl; fall back to hardcoded constants
#         if the file doesn't exist yet (pre-training).
try:
    ...
    # Hardcoded fallback so the app never crashes even before train_models.py is run
    _adj_config = {
    ...
    print(f"[CarIQ] WARNING: market_adjustment_config.pkl not found ({_adj_e})")
    print("[CarIQ]  → Using hardcoded adjustment constants as fallback")


def apply_market_adjustments(base_price, city, body_type):
    """
    Post-prediction multiplier.

    FIX 2: Called by get_fair_price() after the RF prediction so all downstream
    values (price_ratio, fraud ratio, TCO fair price cell) use the adjusted price.

    Returns a dict:
        adjusted_price      — final ₹ after both multipliers
        city_tier           — "Tier 1" / "Tier 2" / "Tier 3"
        city_multiplier     — float
        body_multiplier     — float
        city_adjustment_amt — ₹ delta from city
        body_adjustment_amt — ₹ delta from body type
        total_adjustment_pct— % change vs base
    """
...
try:
    ...
    print("[CarIQ]  → Run module2_train.py to generate models/maintenance_model.pkl")


app = Flask(__name__)
DB_PATH     = os.path.join(os.path.dirname(__file__), "cariq.db")
...
# ══════════════════════════════════════════════════════════════════════════════
#  MODULE 1 — get_fair_price()
#  FIX 2: Now returns ADJUSTED price (city + body type applied post-RF).
#  Also returns the full adjustment breakdown dict via get_fair_price_full().
# ══════════════════════════════════════════════════════════════════════════════

BRAND_CLASS = {
    "Maruti Suzuki": "economy_hr", "Hyundai": "economy_hr", "Tata": "economy_hr",
    "Honda": "mid",    "Toyota": "mid",
    "Mahindra": "premium",
...
def _build_feature_row(listing, vehicle_age=None, km_override=None,
    ...
    owner_num = int(owner_override if owner_override is not None
                    else OWNER_MAP.get(str(owner_raw), 2))

    mb = (mileage_band_override if mileage_band_override is not None
          else _mileage_band(age, km))
...
def get_fair_price_full(listing):
    """
    FIX 2: Returns both the raw RF prediction AND the full adjustment breakdown.
    All callers that need the breakdown (store in DB, fraud detection, TCO)
    call this instead of get_fair_price() directly.

    Returns: (adjusted_price_int, raw_price_int, adjustment_dict)
    """
    ...
    if ML_MODELS_AVAILABLE:
        try:
            ...
            print(f"[CarIQ] Module 1 inference error: {e} — using rule-based fallback")
...
def _rule_based_price(listing):
    """Extracted rule-based fallback so it can be called from get_fair_price_full()."""
...
def get_fair_price(listing):
    """
    Convenience wrapper — returns adjusted price only.
    Used by run_fraud_detection() so ratio is against adjusted price.
    FIX 8: Fraud detection now uses adjusted price for the ratio.
    """
...
def get_maintenance_cost(listing):
    ...
    try:
        ...
        city_tier = ("Metro" if city in _METRO_CITIES
                     else "Tier-3" if city else "Tier-2")
        city_mult = _CITY_LABOUR_MULT.get(city_tier, 1.00)
        final_pred = base_pred * city_mult

        return max(15000, min(int(round(final_pred, -2)), 250000))
...
# ══════════════════════════════════════════════════════════════════════════════
#  MODULE 3 — get_depreciation()
#  FIX 7: Takes adjusted_fair_price so the depreciation % is calculated
#          against the market-adjusted value, not the raw RF output.
# ══════════════════════════════════════════════════════════════════════════════
...
def get_depreciation(listing, adjusted_fair_price):
    ...
    try:
        ...
        X_future = _build_feature_row(
            listing,
            vehicle_age=future_age,
            km_override=future_km,
            owner_override=future_owner,
            mileage_band_override=future_mb,
        )

        # Raw RF future value — then apply same city+body adjustment so
        # current and future values are on the same scale.
        ...
        # FIX 7: depreciation calculated vs adjusted_fair_price
...
# ══════════════════════════════════════════════════════════════════════════════
#  MODULE 4 — Vehicle Condition Detection (OpenCV)
# ══════════════════════════════════════════════════════════════════════════════
...
#  FIX 8: get_fair_price() already returns adjusted price so ratio is correct.
# ══════════════════════════════════════════════════════════════════════════════

def run_fraud_detection(listing, condition_score):
    listed_price = int(listing["price"])
    # get_fair_price() now returns adjusted price (city + body applied)
...
# ══════════════════════════════════════════════════════════════════════════════
#  DATABASE
#  FIX 4: model_outputs table now has adjusted_price, city_tier, body_type,
#          city_multiplier, body_multiplier columns.
# ══════════════════════════════════════════════════════════════════════════════
...
def init_db():
    ...
    # FIX 4: Migrate existing DBs that are missing the new columns
    existing_cols = {
        row[1] for row in conn.execute("PRAGMA table_info(model_outputs)").fetchall()
    }
    new_cols = {
        "adjusted_price":  "INTEGER",
        "city_tier":       "TEXT",
    ...
    for col, typ in new_cols.items():
        if col not in existing_cols:
            conn.execute(f"ALTER TABLE model_outputs ADD COLUMN {col} {typ}")

    listings_cols = {
        row[1] for row in conn.execute("PRAGMA table_info(listings)").fetchall()
    }
    if "body_type" not in listings_cols:
        conn.execute("ALTER TABLE listings ADD COLUMN body_type TEXT NOT NULL DEFAULT 'Other'")

    conn.commit()

    if conn.execute("SELECT COUNT(*) FROM listings").fetchone()[0] == 0:
        # FIX 6: Seeds now include body_type
        seeds = [
        ...
        for s in seeds:
            listing_dict = {
                "brand": s[0], "model": s[1], "year": s[2], "km": s[3],
                "fuel": s[4], "city": s[5], "price": s[6], "transmission": s[7],
                "body_type": s[10],
            }
            cur = conn.execute(
                "INSERT INTO listings (brand,model,year,km,fuel,city,price,transmission,seller_name,owner,body_type) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
            ...
            conn.execute(
                """INSERT INTO model_outputs
                   (listing_id, price_pred, fair_price_rule, price_ratio,
                    ...
                    maintenance_2yr,
                    depreciation_2yr, depreciation_pct, future_value_2yr)
                   VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
...
# ══════════════════════════════════════════════════════════════════════════════
#  HELPERS
#  FIX 5: build_response() now returns all adjustment fields in models dict.
# ══════════════════════════════════════════════════════════════════════════════

def row_to_dict(row):
    return dict(row) if row else {}

def build_response(listing_row, model_row):
    d = row_to_dict(listing_row)
    m = row_to_dict(model_row)
    d["models"] = {
        # Raw RF output
        ...
        # Adjusted price + breakdown (FIX 5)
...
def get_listings():
    ...
    if fuel and fuel != "all":
        ...
        params.append(fuel)
    if query:
        sql += " AND (l.brand LIKE ? OR l.model LIKE ?)"
    ...
    for r in rows:
        ...
        listing = {k: d.get(k) for k in listing_keys}
...
def get_listing(lid):
    ...
    m = conn.execute("SELECT * FROM model_outputs WHERE listing_id=?", (lid,)).fetchone()
    conn.close()
    if not l:
...
def create_listing():
    # FIX 3: Accept body_type from POST body
    ...
    # ── MODULE 3 — pass adjusted_price for correct depreciation % ─────────────
    future_val, depr_abs, depr_pct = get_depreciation(listing_dict, adjusted_price)
    ...
    if image_file and OPENCV_AVAILABLE:
        ...
        with open(image_path, "wb") as f:
            f.write(image_bytes)
        conn.execute("UPDATE listings SET image_path=? WHERE id=?", (fname, lid))
...
def check_condition():
    if not OPENCV_AVAILABLE:
        return jsonify({"error": "OpenCV not installed. Run: pip install opencv-python-headless pillow"}), 503
    image_file = request.files.get("image")
    if not image_file:
...
def update_models(lid):
    ...
    conn = get_db()
    conn.execute(f"UPDATE model_outputs SET {set_clause} WHERE listing_id=?", (*fields.values(), lid))
    conn.commit()
    m = conn.execute("SELECT * FROM model_outputs WHERE listing_id=?", (lid,)).fetchone()
    conn.close()
    return jsonify(row_to_dict(m))

@app.route("/api/status", methods=["GET"])
def status():
    m1_status = "LIVE — Random Forest pkl" if ML_MODELS_AVAILABLE else "FALLBACK — rule-based"
    m2_status = "LIVE — Hist Gradient Boosting pkl" if MODULE2_AVAILABLE else "PENDING — run module2_train.py"
    m3_status = "LIVE — Real-wear simulation RF" if ML_MODELS_AVAILABLE else "PENDING — models/ folder not found"
    adj_status = "LIVE — pkl loaded" if MARKET_ADJ_AVAILABLE else "ACTIVE — hardcoded fallback"
    return jsonify({
        "module_1_price":        m1_status,
        "module_2_maintenance":  m2_status,
        "module_3_depreciation": m3_status,
        "module_4_condition":    "LIVE" if OPENCV_AVAILABLE else "DISABLED — install opencv-python-headless",
        "module_5_fraud":        ("LIVE (RF fair price)" if ML_MODELS_AVAILABLE and OPENCV_AVAILABLE
                                  else "LIVE (rule-based)" if OPENCV_AVAILABLE
                                  else "DISABLED — requires Module 4"),
        "market_adjustments":    adj_status,
